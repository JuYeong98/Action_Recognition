...
class Processor(IO):
    ...
    def __init__(self, argv=None):
        # 초기화 진행 
        self.load_arg(argv) #매개변수 저장
        self.init_environment()  # 실행환경 초기화
        self.load_model() # 모델을 불러온다.
        self.load_weights() # weight 를 불러옴 . weight 는 work_dir/recognition/ucf_skeleton/ST-GCN/에 저장되어있음
        self.gpu()  # gpu 
        self.load_data() #데이터를 불러들임
        self.load_optimizer()  # 옵티마이저 StochasticGradientDescent 사용 

    def init_environment(self):  # 환경설정 초기화

        super().init_environment()
        self.result = dict()  # 결과를 담을 dict 생성
        self.iter_info = dict() # 반복의 정보를 담는 dict생성
        self.epoch_info = dict() # epoch의 정보를 담는 dict생성 
        self.meta_info = dict(epoch=0, iter=0)  # 종합 정보를 담는 dict생성

    def load_optimizer(self): #optimizer를 불러온다 
        pass #왜 pass인지는 모름 . 

    def load_data(self):  #데이터를 불러온다 . 
        Feeder = import_class(self.arg.feeder) #feeder에 있음
    ...
    def get_parser(add_help=False):
        ...
        parser.add_argument('--eval_interval', type=int, default=30, help='the interval for evaluating models (#iteration)')
